...
import { Upload, CheckCircle, AlertTriangle, Loader2, ArrowLeft, ArrowRight, X, FileSpreadsheet } from 'lucide-react'
import { importRelaties } from '@/lib/actions'
import { cn } from '@/lib/utils'

const RELATIE_FIELDS = [
  { value: '', label: '-- Overslaan --' },
  { value: 'bedrijfsnaam', label: 'Bedrijfsnaam *' },
  { value: 'type', label: 'Type (particulier/zakelijk)' },
  ...
  { value: 'opmerkingen', label: 'Opmerkingen' },
]

const COLUMN_ALIASES: Record<string, string[]> = {
  bedrijfsnaam: ['bedrijfsnaam', 'bedrijf', 'naam', 'company', 'name', 'organisatie', 'organization', 'klantnaam', 'relatienaam'],
  type: ['type', 'soort', 'klanttype', 'relatietype'],
  ...
  kvk_nummer: ['kvk', 'kvk_nummer', 'kvknummer', 'coc', 'kvk nummer'],
  btw_nummer: ['btw', 'btw_nummer', 'btwnummer', 'vat', 'btw nummer'],
  iban: ['iban', 'bankrekeningnummer', 'bankrekening', 'rekeningnummer'],
  website: ['website', 'url', 'web', 'homepage'],
  opmerkingen: ['opmerkingen', 'notities', 'notes', 'opmerking', 'memo'],
}
...
export function ImportRelatiesDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function handleImport() {
    ...
    const result = await importRelaties(rows)
    if ('error' in result) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} title="Relaties importeren" className="max-w-3xl">
      {/* Step indicator */}
        ...
        <div>
          <div className="bg-blue-50 border border-blue-200 rounded-md p-3 mb-4">
            <p className="text-sm text-blue-800">
              <strong>{validRows}</strong> relaties gevonden
                ...
                <tr className="bg-gray-50 border-b border-gray-200">
                  {Object.values(mapping).map(field => (
                    <th key={field} className="text-left text-xs font-medium text-gray-500 uppercase px-4 py-2 whitespace-nowrap">
                      {RELATIE_FIELDS.find(f => f.value === field)?.label || field}
                    </th>
                  ))}
                </tr>
              </thead>
        ...
        <div className="space-y-4">
          {importResult.imported > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-green-800">{importResult.imported} relaties succesvol geimporteerd</p>
              </div>
            </div>
          )}
