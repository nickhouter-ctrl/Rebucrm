      ...
      if (metaDoc) {
        try {
          ...
          if (pdfFile) {
            ...
            // Extract element data and total from text
            const elementData = parseElementsFromText(parsed.text)
...
function parseTotaalFromText(text: string): number {
  ...
  if (isGealan) {
    ...
    if (totaalMatch) {
      return parseFloat(totaalMatch[1].replace(/\s/g, '').replace(/\./g, '').replace(',', '.'))
    }
  } else {
