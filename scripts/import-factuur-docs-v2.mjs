...
function parseBedrag(str) {
  // Nederlands formaat: 1.524,677 → 1524.677
...
function mapFactuurType(onderwerp) {
  ...
  if (lower.includes('credit')) return 'volledig'
  return 'volledig'
}
...
async function main() {
  ...
  // Check of er al geïmporteerde facturen zijn (voorkom duplicaten)
  const { data: bestaandeFacturen } = await supabase
    .from('facturen')
    .select('factuurnummer')
    .eq('administratie_id', adminId)

  const bestaandeNummers = new Set((bestaandeFacturen || []).map(f => f.factuurnummer))
  console.log(`${bestaandeNummers.size} bestaande facturen in DB`)
  ...
  for (const row of facturen) {
    ...
    const btwTotaal = totaalIncl - totaalExcl

    // Zoek of maak relatie
    let relatie = null
    if (relatieNaam) {
    ...
    // Zoek project via meerdere strategieën
    let project = null
    ...
    // Strategie 3: Nog geen project → maak nieuw aan
    if (!project) {
    ...
    // Maak factuurrecord aan (als die nog niet bestaat)
    const formattedNummer = factuurnummer.startsWith('F-') ? factuurnummer : `F-${factuurnummer}`
    if (!bestaandeNummers.has(factuurnummer) && !bestaandeNummers.has(formattedNummer)) {
      ...
      const { error: factError } = await supabase
        ...
        .insert({
          ...
          subtotaal: totaalExcl,
          btw_totaal: btwTotaal > 0 ? btwTotaal : 0,
          totaal: totaalIncl,
          betaald_bedrag: status === 'betaald' ? totaalIncl : 0,
      ...
      if (factError) {
        ...
        bestaandeNummers.add(factuurnummer)
        factuurAangemaakt++
      }
    } else {
      factuurOvergeslagen++
  ...
  console.log('Facturen overgeslagen (bestond al):', factuurOvergeslagen)
  console.log('Projecten gematcht (bestaand):', projectGematcht)
  console.log('Projecten nieuw aangemaakt:', projectAangemaakt)
  console.log('PDFs geupload:', pdfUploaded)
  console.log('PDFs overgeslagen (duplicaat):', pdfSkipped)
  console.log('Fouten:', errors)

  // Check totalen
  const { count: totaalFacturen } = await supabase
    .from('facturen')
    .select('id', { count: 'exact', head: true })
    .eq('administratie_id', adminId)

  const { count: totaalProjecten } = await supabase
    .from('projecten')
    .select('id', { count: 'exact', head: true })
    .eq('administratie_id', adminId)

  console.log('\n--- Totalen in DB ---')
  console.log('Totaal facturen:', totaalFacturen)
  console.log('Totaal projecten:', totaalProjecten)
}
